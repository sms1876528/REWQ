...
/**
 * ============================================================================
 * 단계 3: JSON 처리 + HTTP API 서버 (네트워크 통신) - 멀티 스레드
 * ============================================================================
 *
 * SUB3 ProgramMulti - HTTP API 서버 (멀티 스레드 버전)
 *
 * [학습 목표]
 * - 멀티 스레드 프로그래밍 (Executors.newCachedThreadPool)
 * - 스레드 안전성 (synchronized, lock)
 * - 불변 객체 vs 가변 객체 관리
 * - 스냅샷 패턴 (데이터 복사를 통한 안전한 읽기)
 * - 클래스 분리와 캡슐화 (MonitoringStore, PerformanceCalculator)
 *
 * [ProgramSingle과의 차이점]
 * - Single: 한 번에 1개 요청만 처리 → Multi: 여러 요청 동시 처리
 * - Single: 동기화 불필요 → Multi: synchronized로 데이터 보호 필수
 * - Single: 전역 변수 직접 사용 → Multi: 클래스로 캡슐화하여 안전성 확보
 *
 * [프로그램 목적]
 ...
 *
 * [제공 API]
 * 1. POST /monitoring: 모니터링 데이터 수집
 * 2. POST /performance: 모델별 성능 조회 (정확도)
 *
 * [동작 방식]
 ...
 *
 * [다음 단계로]
 * - SUB4: 성능 메트릭 확장 (latency 평균값 계산 추가)
 */
public class ProgramMulti {
    ...
    /**
     * 성능 계산기
     * - MonitoringStore를 사용하여 성능 통계 계산
     */
    static class PerformanceCalculator {
        ...
        String performance(String modelName, String timeWindow) {
            ...
            Map<String, Integer> pred = new HashMap<>();  // 예측값
